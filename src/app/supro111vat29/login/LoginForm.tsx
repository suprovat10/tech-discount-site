...
export function LoginForm({ passwordLoginEnabled = true }: LoginFormProps) {
  ...
  if (!displayError && queryError) {
    if (queryError === 'google_not_configured') {
      displayError =
        'Google Sign-In is not configured yet. Please add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in Vercel settings.';
  ...
  return (
    <div className="w-full max-w-md relative z-10">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm font-medium text-slate-400 mt-1">Admin Console Verification</p>
      </div>
      ...
      <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 sm:p-8 shadow-2xl">
        ...
        <a
          ...
          className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white hover:bg-slate-100 text-slate-900 font-bold text-sm rounded-xl shadow-lg shadow-white/5 transition-all cursor-pointer"
        >
        ...
        {/* Username & Password Login Form (conditionally rendered) */}
        {passwordLoginEnabled ? (
          ...
          <div className="mt-5 p-3 rounded-xl bg-blue-500/10 border border-blue-500/20 text-center">
            <p className="text-xs text-blue-300 font-medium">
              Password login is temporarily disabled. Please sign in using your authorized Google account.
            </p>
          </div>
        )}
