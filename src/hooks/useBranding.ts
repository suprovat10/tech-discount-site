...
// Module-level cache so multiple components don't make duplicate network calls
let cachedServerSettings: any = null;
let isFetchingSettings = false;
let fetchListeners: Array<(data: any) => void> = [];

function fetchSettingsDeduplicated(callback: (data: any) => void) {
  if (cachedServerSettings) {
    callback(cachedServerSettings);
    return;
  }
    ...
    fetch('/api/settings')
      ...
      .then((serverData) => {
        if (serverData && !serverData.error && Object.keys(serverData).length > 0) {
          cachedServerSettings = serverData;
          fetchListeners.forEach((cb) => cb(serverData));
        }
      })
      .catch(() => {})
      .finally(() => {
...
export function useBranding() {
  ...
  useEffect(() => {
    ...
    // Sync with server-side saved settings (deduplicated & deferred)
    fetchSettingsDeduplicated((serverData) => {
      if (serverData && !serverData.error && Object.keys(serverData).length > 0) {
